...
module operational where
  ...
  ↑[]-var-shift : {n m x : ℕ} {d : ℤ} (le1 : n Data.Nat.≤ x) (le2 : x Data.Nat.≤ m) → ↑[ n , m ] d [ Var x ] ≡  Var (∣ (ℤ.pos x) Data.Integer.+ d ∣)
  ...
  ↑¹-var {zero} = refl
  ↑¹-var {ℕ.suc x}
    rewrite (sym (n+1≡sucn{x Data.Nat.+ 1}))
          | (sym (n+1≡sucn{x}))
          = cong ↑¹[_] (↑¹-var{x})
  ...
  swap-type : {Δ ∇ Γ : Env} {T : Ty} → (length Δ) ∶ T ∈ (Δ ++ T ∷ ∇ ++ Γ) → (length Δ Data.Nat.+ length ∇) ∶ T ∈ (Δ ++ ∇ ++ T ∷ Γ)
  swap-type {Δ} {∇} {Γ} {T} j
  ...
  ---- lemmas and properties required for subst-swap lemma

  -- some very specific calculations req. for swap-subst var case
  module auxiliary-calculations where
    ...
    minus-1 : {y x : ℕ} {lt : x Data.Nat.< y} → ∣ y ∸ x ⊖ 1 ∣ Data.Nat.+ x ≡ ∣ y ⊖ 1 ∣
    minus-1 {y} {x} {lt}
      rewrite (⊖-≥{y ∸ x}{1} (m>n⇒m∸n≥1 lt))
            | (sym (+-∸-comm {y ∸ x} x {1} (m>n⇒m∸n≥1 lt)))
            | (m∸n+n≡m {y} {x} (sucn≤m⇒n≤m lt))
            | (⊖-≥{y}{1} ((m∸n≥q⇒m≥q{y}{x} (m>n⇒m∸n≥1{y}{x} lt))))
            = refl

  open auxiliary-calculations

  -- swap-subst lemma: swap the position of a type in an environment
  ...
  swap-subst {(Fun T₁ T₂)} {S} {Γ} {Δ} {∇} {(Abs e)} {r} {gt} (TAbs j)
    rewrite (↑¹-var{length Δ Data.Nat.+ length ∇})
          | (n+1≡sucn{r})
  ...
  swap-subst {T} {S} {Γ} {Δ} {∇} {Var y} {r} {gt} (TVar j)
    with extract{Δ} {(S ∷ ∇) ++ Γ} {T} {y} j
       | y Data.Nat.<? (foldr (λ _ → ℕ.suc) 0 Δ)
       | Data.Nat._≟_ y (length Δ)  
  -- y ∈ Δ
  ... | in-Δ x | yes p | yes q = contradiction q (<⇒≢ p)
  ... | in-Δ x | yes p | no ¬q
    rewrite (↑[]-var-refl-<{ℕ.suc (length Δ)}{length Δ Data.Nat.+ length ∇}{y}{ -[1+ 0 ] }{[k<x]⇒[k<sucx] p})
  ...
  swap-subst {T} {S} {Γ} {Δ} {∇} {Var y} {r} {gt} (TVar j) | in-Γ ge x | p | yes q' | in-Δ x'
    rewrite (↑[]-var-refl->{ℕ.suc (length Δ)}{length Δ Data.Nat.+ length ∇}{r}{ -[1+ 0 ] }{<-trans (s≤s (n+m≤n+q+m {length Δ} {length ∇} {length Γ})) gt})
          | (subst-trivial{r} {Var (length Δ Data.Nat.+ length ∇)})
          | q'
          | (env-type-equiv j)
          = TVar (swap-type{Δ}{∇}{Γ}{S} (env-type-equiv-j (env-type-equiv j) j))
  ---- y ∈ ∇
  swap-subst {T} {S} {Γ} {Δ} {∇} {Var y} {r} {gt} (TVar j) | in-Γ ge x | p | no ¬q' | in-Δ x'
    rewrite (↑[]-var-shift{ℕ.suc (length Δ)}{length Δ Data.Nat.+ length ∇}{y}{ -[1+ 0 ]} (≤∧≢⇒< ge (≢-sym ¬q')) (≤-trans (≤-pred (m≤n∧m≡q⇒q≤n (m≤n⇒m+o≤n+o{o = length Δ} (var-env-< x'))
            (cong ℕ.suc (m∸n+n≡m ge)))) (≤-refl-+-comm{length ∇}{length Δ})))
          | length-≡ {Δ} {∇} {Γ} {S}
          | (var-subst-refl {r} {∣ y ⊖ 1 ∣} {≢-sym (<⇒≢ (<-trans (n>0⇒n>∣n⊖1∣ (≤-trans (s≤s z≤n) ((≤∧≢⇒< ge (≢-sym ¬q'))))) (<-trans (var-env-< j) gt)))} {Var (length Δ Data.Nat.+ length ∇)})
