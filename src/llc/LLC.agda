...
open import Data.Vec renaming (length to lengthᵛ ; _++_ to _++ᵛ_ ; foldr to foldrⱽ)
open import Data.Vec.Relation.Unary.All renaming (All to Allᵛ)
...
module defs where
  data Exp : Set where
    ...
    LabEl : {n : ℕ} {s : Subset n} → (∀ l → l ∈ s → Exp) → Exp → Exp
  
  data Ty : Set where
    Fun : Ty → Ty → Ty
    Label : {n : ℕ} → Subset n → Ty
  ...
  data _⊢_∶_ : Env → Exp → Ty → Set where
    ...
    TLabI : {Γ : Env} {n : ℕ} {x : Fin n} {s : Subset n} → (ins : x ∈ s) → Γ ⊢ LabI ins ∶ Label {n} s
    TLabEl : {Γ : Env} {T : Ty} {n : ℕ} {s : Subset n} {e : Exp} {f : ∀ l → l ∈ s → Exp} → (∀ l i → (Γ ⊢ (f l i) ∶ T)) → Γ ⊢ e ∶ Label {n} s → Γ ⊢ LabEl {n} {s} f e ∶ T
...
module denotational where
  ...
  Val (Label {n} s) = Σ (Fin n) (λ x → x ∈ s)

  access : {n : ℕ} {Γ : Env} {T : Ty} → n ∶ T ∈ Γ → All Val Γ → Val T
  access here (V ∷ Γ) = V
  access (there J) (V ∷ Γ) = access J Γ

  eval : {Γ : Env} {T : Ty} {e : Exp} → Γ ⊢ e ∶ T → All Val Γ → Val T
...
module operational where
  ...
  ↑ d , c [ LabEl f e ] = LabEl (λ l x → ↑ d , c [ (f l x) ]) (↑ d , c [ e ])  
  ...
  ↑[ n , m ] d [ LabEl f e ] = LabEl (λ l x → ↑[ n , m ] d [ (f l x) ]) (↑[ n , m ] d [ e ])
  ...
  [ k ↦ s ] LabEl f e = LabEl (λ l x → [ k ↦ s ] (f l x)) ([ k ↦ s ] e)

  data Val : Exp → Set where
    VFun : {e : Exp} → Val (Abs e)
    VLab : {n : ℕ} {x : Fin n} {s : Subset n} {ins : x ∈ s} → Val (LabI ins)

  -- reduction relation
  data _⇒_ : Exp → Exp → Set where
    ξ-App1 : {e₁ e₁' e₂ : Exp} → e₁ ⇒ e₁' → App e₁ e₂ ⇒ App e₁' e₂
    ξ-App2 : {e e' v : Exp} → Val v → e ⇒ e' → App v e ⇒ App v e'
    ξ-LabEl : {e e' : Exp} {n : ℕ} {s : Subset n} {f : ∀ l → l ∈ s → Exp} → e ⇒ e' → LabEl f e ⇒ LabEl f e'
    β-App : {e v : Exp} → Val v → (App (Abs e) v) ⇒ (↑⁻¹[ ([ 0 ↦ ↑¹[ v ] ] e) ])
    β-LabEl : {n : ℕ} {s : Subset n} {f : ∀ l → l ∈ s → Exp} {x : Fin n} → (ins : x ∈ s) → LabEl f (LabI ins) ⇒ f x ins
  ...
  ↑⁻¹ₖ[↑¹ₖ[s]]≡s {LabI ins} = refl
  ↑⁻¹ₖ[↑¹ₖ[s]]≡s {LabEl f e} = cong₂ LabEl (f-ext (λ x → f-ext (λ ins →  ↑⁻¹ₖ[↑¹ₖ[s]]≡s)))  ↑⁻¹ₖ[↑¹ₖ[s]]≡s
  ...
  ↑ᵏ[↑ˡ[s]]≡↑ᵏ⁺ˡ[s] {k} {l} {c} {LabI ins} le = refl
  ↑ᵏ[↑ˡ[s]]≡↑ᵏ⁺ˡ[s] {k} {l} {c} {LabEl f e} le = cong₂ LabEl (f-ext (λ x → f-ext (λ ins → ↑ᵏ[↑ˡ[s]]≡↑ᵏ⁺ˡ[s] {k} {l} {c} {f x ins} le))) ( ↑ᵏ[↑ˡ[s]]≡↑ᵏ⁺ˡ[s] {k} {l} {c} {e} le)
  ...
  ↑k,q[↑l,c[s]]≡↑l+k,c[s] {k} {l} {q} {c} {LabI e} ge₁ ge₂ = refl
  ↑k,q[↑l,c[s]]≡↑l+k,c[s] {k} {l} {q} {c} {LabEl f e} ge₁ ge₂ = cong₂ LabEl (f-ext (λ x → f-ext (λ ins → ↑k,q[↑l,c[s]]≡↑l+k,c[s] {k} {l} {q} {c} {f x ins} ge₁ ge₂))) (↑k,q[↑l,c[s]]≡↑l+k,c[s] {k} {l} {q} {c} {e} ge₁ ge₂)
  ...
  ext {Γ} {Δ} {∇} {S} {LabI ins} (TLabI{n = n}{x}{s} .ins) = TLabI{n = n}{x}{s} ins
  ext {Γ} {Δ} {∇} {S} {LabEl f e} (TLabEl{f = .f} f' j) = TLabEl (λ l i → ext{Γ}{Δ}{∇} (f' l i)) (ext{Γ}{Δ}{∇} j)
  ...
  progress (LabEl{n}{s} f e) {T} {TLabEl{n = .n}{s = .s}{f = .f} f' j}
    with progress e {j = j}
  ... | step{e'} x = step (ξ-LabEl{e}{e'}{n}{f = f} x)
  progress (LabEl {n} {s} f .(LabI ins)) {T} {TLabEl {_} {_} {.n} {.s} {_} {.f} f' (TLabI .ins)}
  ...
  preserve (LabEl f e) .(LabEl f _) (TLabEl f' j) (ξ-LabEl{.e}{e'} r) = TLabEl f' (preserve e e' j r)
  preserve (LabEl f .(LabI ins)) .(f _ ins) (TLabEl f' j) (β-LabEl{x = x} ins) = f' x ins
